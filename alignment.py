...
def alignPair(f1, f2, matches, m, nRANSAC, RANSACthresh):
    ...
    num_samples = 1 if (m == eTranslate) else 4
    ...
    for i in range(nRANSAC):
        ...
        if m==eTranslate:
            (ax, ay) = f1[matches[0].queryIdx].pt
            (bx, by) = f2[matches[0].trainIdx].pt
            homography[0,2] = bx - ax
            homography[1,2] = by - ay
        else:
            homography = computeHomography(f1,f2,samples)
        inlier_indices = getInliers(f1,f2,matches,homography,RANSACthresh)
        if len(inlier_indices) > len(indices):
            indices = inlier_indices
    M = leastSquaresFit(f1, f2, matches, m, indices)
    return M
...
def getInliers(f1, f2, matches, M, RANSACthresh):
    ...
    for i in range(len(matches)):
        ...
        #TODO-BLOCK-BEGIN
        m = matches[i]
        (ax, ay) = f1[m.queryIdx].pt
        (bx, by) = f2[m.trainIdx].pt
        point = np.array([ax,ay,1])
        mapped_pt = M @ point
        mapped_pt_pred1 = mapped_pt[0]/mapped_pt[2] #x/z
        mapped_pt_pred2 = mapped_pt[1]/mapped_pt[2] #y/z
        euclidian_dist = np.sqrt((bx - mapped_pt_pred1)**2 + (by-mapped_pt_pred2)**2)
        if euclidian_dist < RANSACthresh:
...
def leastSquaresFit(f1, f2, matches, m, inlier_indices):
    ...
    if m == eTranslate:
        ...
        for inlier_idx in inlier_indices:
            inlier_matches.append(matches[inlier_idx])

        M = computeHomography(f1, f2, inlier_matches, None)
        #TODO-BLOCK-END
        #END TODO
