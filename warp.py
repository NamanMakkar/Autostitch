...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    # TODO-BLOCK-BEGIN
    x = np.sin(xf) * np.cos(yf) # x_cap = sin(theta)*cos(phi)
    y = np.sin(yf) #y_cap = sin(phi)
    z = np.cos(xf) * np.cos(yf) #z_cap = cos(theta) * sin(phi)
    x = x/z
    y = y/z
    #radial distortion calculation
    r = np.sqrt(x**2 + y**2)
    xt = x*(1+k1*r**2+k2*r**4)
    yt = y*(1+k1*r**2+k2*r**4)
    # TODO-BLOCK-END
    # END TODO
